...
export function formatTimesheetDetail(item: timesheet.TimesheetDetail): string {
  ...
  if (item.timesheetRecords.length > 0) {
    ...
    for (const rec of item.timesheetRecords) {
      // Prefer the server-rendered `hours` string; fall back to deriving
      // from `duration` (string minutes) when the server omits it.
      const hours = rec.hours ?? (Number(rec.duration) / 60).toFixed(2);
