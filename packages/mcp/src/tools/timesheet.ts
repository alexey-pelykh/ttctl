  ...
  server.registerTool(
    "ttctl_timesheet_list",
    {
      ...
      description: [
        ...
        "compatibility.",
        "",
        "Each row carries approval state (`timesheetApproved`,",
        "`timesheetRequiresApproval`) alongside `timesheetSubmitted`, so a",
        "caller can tell 'awaiting the client' from 'approved' without a",
        "per-cycle detail call.",
        "",
  ...
  server.registerTool(
    "ttctl_timesheet_pending_list",
    {
      ...
      description: [
        ...
        "a too-narrow window.",
        "",
        "**'Pending' here means pending SUBMISSION (by you), NOT pending",
        "approval (by the client).** This tool lists cycles you still need to",
        "submit. For approval state, read `timesheetApproved` /",
        "`timesheetRequiresApproval` on any cycle via `ttctl_timesheet_list`",
        "or `ttctl_timesheet_show`.",
        "",
  ...
  server.registerTool(
    "ttctl_timesheet_show",
    {
      ...
      description: [
        ...
        "(week range, hours, submission + approval state), the engagement+job",
        "reference, the rate agreement snapshot, the comment (if any), and the",
        "per-day time entries (`timesheetRecords`).",
        "",
        "Approval state: `timesheetApproved` (the client signed off),",
        "`timesheetRequiresApproval` (whether sign-off is needed at all), and",
        "the raw billing-cycle `status` — distinguishes 'submitted, awaiting",
        "the client' from 'approved'.",
        "",
        "Example user prompts:",
