  ...
  server.registerTool(
    "ttctl_surveys_list",
    {
      ...
      description: [
        ...
        "",
        "`inputType` — NOT whether `answers[]` is empty — drives the answer value. Never infer the",
        "type from `answers[]`: an `OPEN_TEXT` question can arrive carrying a single sentinel option,",
        "so keying on emptiness misclassifies it. `ttctl_surveys_submit` documents the per-type values.",
        "",
  ...
  server.registerTool(
    "ttctl_surveys_submit",
    {
      ...
      description: [
        ...
        "`questions[]`. Answer EVERY question with `isMandatory: true` — omitting one is rejected.",
        "",
        "Optional questions are NOT free to skip: submitting CLOSES the survey, so anything left out is",
        "forfeited permanently and cannot be added later. Answer them too, and if the user has not said",
        "what to put, ASK before submitting rather than silently dropping them — ttctl warns about",
        "omissions on stderr, which this tool's response does not carry back to you.",
        "",
        "For each question pass `{ questionId, value }`, keyed off its `inputType` — never off",
        "whether `answers[]` is empty, which does not classify the question:",
        "  - `RADIO_BUTTONS` / `RATING` / `SLIDER` → `value` is the chosen option's `value`",
        '  - `CHECKBOX` → `value` is `"true"` or `"false"`. Read the question `label` to know what',
        '    `"true"` asserts (label "This interview didn\'t occur." → `"true"` means it did NOT occur).',
