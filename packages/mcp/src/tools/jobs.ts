  ...
  server.registerTool(
    "ttctl_jobs_apply_questions",
    {
      ...
      description: [
        ...
        "`{ matcherQuestions: [...], expertiseQuestions: [...] }`. Each",
        "entry carries `identifier`, `prompt` (the question text),",
        "`type` (`matcher` | `expertise`), `isMandatory`, plus the",
        "choice metadata: `options` (the dropdown's allowed values — an",
        "empty array for free-text questions), `suggestedAnswer` (the",
        "recruiter-preselected value, or null), and `inputType`",
        "(`dropdown` when `options` is non-empty, else `free-text`).",
        "For a dropdown matcher question, answer with one of `options`",
        "(or the `suggestedAnswer` if it fits); for `free-text`, any",
        "string. (Expertise questions are answered via profile-item",
        "selection — they carry empty `options` / null `suggestedAnswer`.)",
        "",
        "Read-only — does not mutate any server state.",
        "",
