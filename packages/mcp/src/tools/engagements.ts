  ...
  server.registerTool(
    "ttctl_engagements_breaks_list",
    {
      ...
      description: [
        ...
        "",
        "Each break entry carries the `reason` field (per #346) — the same",
        "shape returned by `ttctl_engagements_breaks_reasons_list`:",
        "`{ identifier, nameForRole }`. The `identifier` round-trips the value",
        "passed to `ttctl_engagements_breaks_add`'s `reasonIdentifier` so",
        "an agent can determine why a specific break was scheduled (vacation,",
        "client on vacation, other, etc.) without joining against the catalog.",
        "",
        "Example user prompts:",
        '  - "Show me the scheduled breaks on engagement act_xyz."',
        '  - "Do I have any vacations planned on my current Toptal engagement?"',
        '  - "Which of my scheduled breaks are tagged as vacation?"',
      ].join("\n"),
      inputSchema: {
        id: z.string().describe("Engagement id (the TalentJobActivityItem id from `engagements_list`)"),
        dryRun: DRY_RUN_FIELD,
