 ...
 *   - Rejection/approval *workflow* — the act of approving/rejecting is
 *     client-side and unavailable to talent. Approval *status* IS
 *     read-projected (#849): `timesheetApproved` /
 *     `timesheetRequiresApproval` / `status` ride the shared
 *     `timesheetListFields` fragment onto list + detail.
 */
...
export interface TimesheetListItem {
  ...
  /**
   * Approval state, selected on `timesheetListFields`.
   * `timesheetApproved` = client signed off; `timesheetRequiresApproval`
   * = whether sign-off is needed at all; `status` = raw BillingCycle
   * lifecycle enum.
   */
 ...
 * day off and typically carry `duration === "0.0"`. `hours` is the
 * server-rendered hour form of `duration` (e.g. `"8.0"`), so callers can
 * render it verbatim rather than dividing minutes. `persisted` flags a
 * saved record vs an unsaved draft day-slot — fresh cycles pre-populate
 * `persisted: false` rows.
 *
