...
/**
 * E2E coverage for `ttctl applications interview show <id>` (#439).
 *
 * **Mandatory per CLAUDE.md § Schema/contract validation rule** — the
 * `Interview` op is in `GATEWAY_MOBILE_KNOWN_UNTRUSTED_OPS`
 ...
 *   - Detail projection for a real interview id (discovered via
 ...
 * Mirrors `61-payments-summary.e2e.test.ts` — used by the snapshot test
 * to call the service-layer fn directly (the CLI/MCP-level invocation
 * runs the same projection, so either path captures the same shape;
 * service-level call sidesteps the JSON envelope wrap).
 */
 ...
 * Activity items carry an `interview: { id } | null` field; the first
 * non-null id is sufficient for the detail probe.
 *
 * Returns `null` when:
 ...
 * `Interview` op; the snapshot is opportunistic (captures when
...
async function discoverInterviewId(cli: CliClient): Promise<string | null> {
  ...
  if (result.exitCode !== 0) {
    process.stdout.write(
      `[62-applications-interview-show] discovery via applications list failed (exitCode=${result.exitCode.toString()}); detail+snapshot tests will skip. stdout=${result.stdout || "(empty)"}\n`,
    );
    return null;
  }
...
describe("applications interview show (live mobile-gateway)", () => {
  ...
  it.skipIf(!e2eEnabled)("returns a structured NOT_FOUND error for an unknown interview id", async () => {
    ...
    const result = await cli.run(["applications", "interview", "show", fakeId, "-o", "json"]);
  ...
  it.skipIf(!e2eEnabled)("returns the detail projection for a real interview id (discovered from `list`)", async () => {
    ...
    if (id === null) {
      ...
      process.stdout.write(
        "[62-applications-interview-show] No interview in activity list; skipping detail+snapshot tests.\n",
      );
      return;
    }

    const result = await cli.run(["applications", "interview", "show", id, "-o", "json"]);
    ...
    // null on the wire (sparse interview row); the keys themselves
    // must be present.
    for (const key of [
      "id",
      "status",
  ...
  // -------------------------------------------------------------------
  // Wire-shape snapshot assertion (T1 disposition; #439).
  //
  // Track 1 continuous-detection defense for the `Interview` op against
  // post-merge wire drift (a field renamed / removed / retyped, or the
  // method/contacts/notes/guide sub-shapes changing structure). Captured
  // against the projected `InterviewDetail` — the surface CLI/MCP
  // consumers depend on. Snapshot lives at
  // `wire-snapshots/Interview.snapshot.json`; the first authenticated
  // run with `TTCTL_UPDATE_WIRE_SNAPSHOTS=1` captures it.
  //
  ...
  // -------------------------------------------------------------------
  it.skipIf(!e2eEnabled)("Interview wire shape matches snapshot", async () => {
    const id = await discoverInterviewId(cli);
    if (id === null) {
      process.stdout.write(
        "[62-applications-interview-show] No interview in activity list; skipping wire-shape snapshot.\n",
      );
      return;
    }

    const token = loadSandboxBearer(sandboxConfigPath);
    const response = await applications.interviews.show(token, id);
    expect(() =>
